package com.library.library.game.demo.fptree;
...
public class FPTree {
	...
	public void FPGrowth(List<List<String>> transactions,
		...
		for (TNode head : headerTable) {
			...
			while (nextnode != null) {
				...
				while ((count--) > 0) {
					newTransaction.add(parentNodes);// 添加模式基的前缀 ，因此最终的频繁项为:
													// parentNodes ->
													// newPostPattern
				}
				// 下一个同名节点
	 ...
	 * @return
	 */
	public TNode bulidFPTree(List<TNode> headertable,
	 ...
	 *            被添加的子树
	 */
	public void addSubTree(List<TNode> headertable, TNode subtreeRoot,
			LinkedList<String> itemsDesc) {
		if (itemsDesc.size() > 0) {
	...
	// 将items按count从高到低排序
	public LinkedList<String> sortItemsByDesc(List<String> items,
			List<TNode> headertable) {
	...
	public static void main(String[] args) {
		...
		List<List<String>> transactions = fptree
				.loadTransaction("C:\\Users\\shimin\\Desktop\\新建文件夹\\wordcounts.txt");
